// Lógica real de la sincronización con Google Calendar — separada de los
// endpoints (sync-calendarios-ocupados.js y sync-calendarios-ahora.js) para
// que la use tanto la corrida programada como el botón manual. Este archivo,
// al vivir en una subcarpeta (lib/), no se registra solo como una función
// más — Netlify solo convierte en endpoint a los .js que están directo
// dentro de netlify/functions/.
...
// Punto de entrada único que usan los dos endpoints. Devuelve un objeto
// plano — cada endpoint lo envuelve en su propia respuesta HTTP.
async function realizarSyncCalendarios() {
  try {
    const auth = await obtenerAccessTokenValido();
    if (!auth) {
      return { ok: true, motivo: "no_conectado" };
    }
    ...
    return { ok: true, calendarios: calendarIds.length, horarios: filas.length };
  } catch (err) {
    console.error("Error inesperado en la sincronización de calendarios:", err);
    await sbRpc("registrar_error_sync_google", { p_error: String(err).slice(0, 500) }).catch(() => {});
    return { ok: false, error: String(err) };
  }
}

module.exports = { realizarSyncCalendarios };
